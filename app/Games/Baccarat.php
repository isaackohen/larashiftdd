...
class Baccarat extends MultiplayerGame
{
    ...
    public function customWagerCalculation(Data $data): ?bool
    {
        ...
        if ($totalBet < floatval(Settings::get('min_bet') / Currency::find($data->currency())->tokenPrice()) || ($data->user() != null && $data->user()->balance(Currency::find($data->currency()))->demo($data->demo())->get() < $totalBet)) {
            return false;
        }
        $data->bet($totalBet);
